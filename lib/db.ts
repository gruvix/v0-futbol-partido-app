...
export async function initializeDatabase() {
  ...
  try {
    ...
    // Email auth: nullable for existing users, required for new registrations going forward.
    await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS email VARCHAR(255)`
    await sql`ALTER TABLE pending_users ADD COLUMN IF NOT EXISTS email VARCHAR(255)`
  } catch {
    // Columns might already exist
  }

  // Case-insensitive uniqueness on email (nullable column: multiple NULLs allowed).
  try {
    await sql`
  ...
  await sql`ALTER TABLE matches ADD COLUMN IF NOT EXISTS field_id INTEGER REFERENCES fields(id)`
  await sql`CREATE INDEX IF NOT EXISTS idx_matches_field_id ON matches(field_id)`

  await seedFieldsIfEmpty()
  await backfillMatchFieldIds()

  return { success: true }
}

async function seedFieldsIfEmpty(): Promise<void> {
  const existing = await sql`SELECT id FROM fields LIMIT 1`
  ...
  await sql`
    ...
    VALUES
      (
        'Terrazas',
        'terrazas',
        'Cancelar con al menos 24h vía ATC; después se pierde la seña / se cobra el turno.',
        'Castex 1745.',
        'terrazas-bariloche'
      ),
      (
        'Fénix',
        'fenix',
        'Cancelar con al menos 24h; fuera de plazo se cobra el alquiler completo.',
        'Confirmar dirección/maps con admins.',
        NULL
      ),
      (
        'Los Andes',
        'los-andes',
        'Cancelar ≥24h antes; tardío = 100% del turno.',
        'Confirmar política/maps con predio.',
        NULL
      ),
      (
        'Otra ubicación',
        'otro',
        NULL,
        'Predio no registrado; completar nombre al crear el partido.',
        NULL
      )
  `
}

async function backfillMatchFieldIds(): Promise<void> {
  await sql`
    UPDATE matches m
    SET field_id = f.id
    FROM fields f
